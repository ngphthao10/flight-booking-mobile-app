    ...
    try:
        ...
        if request.method == "POST":
            ...
            departure_date = datetime.strptime(
                request.form.get("departure-date"), "%Y-%m-%d"
            ).strftime("%d-%m-%Y")
            ...
            if is_round_trip and return_date:
                search_data["ngay_ve"] = datetime.strptime(
                    return_date, "%Y-%m-%d"
                ).strftime("%d-%m-%Y")
            else:
                search_data["ngay_ve"] = ""
            ...
            response = requests.post(
                f"{current_app.config['API_URL']}/api/flights/search", json=search_data
            )
            response.raise_for_status()
            flight_results = response.json()
            print("kg:", flight_results)
            if flight_results.get("direct_flights"):
                processed_results["direct_flights"].extend(
                    flight_results["direct_flights"]
                )
            if flight_results.get("connecting_flights"):
                processed_results["connecting_flights"].extend(
                    flight_results["connecting_flights"]
                )
            if flight_results.get("return_direct_flights"):
                processed_results["return_direct_flights"].extend(
                    flight_results["return_direct_flights"]
                )
            if flight_results.get("return_connecting_flights"):
                processed_results["return_connecting_flights"].extend(
                    flight_results["return_connecting_flights"]
                )
            ...
            search_params = {
                ...
                "return_date": (
                    datetime.strptime(return_date, "%Y-%m-%d").strftime("%d-%m-%Y")
                    if return_date
                    else None
                ),
                "adults": adults,
        ...
        try:
            sanbay_response = requests.get(
                f"{current_app.config['API_URL']}/api/sanbay"
            )
        ...
        if request.args.get("filtered"):
            ...
            try:
                filter_response = requests.post(
                    ...
                    json={"flights": processed_results, "filters": filter_params},
                )
...
def min_price(flights):
    ...
    try:
        return min(
            flight.get("gia_ve_eco", float("inf")) for flight in flights if flight
        )
    except:
        return 0
...
def min_duration_price(flights):
    ...
    try:
        valid_flights = [
            ...
            if flight and flight.get("thoi_gian_bay", float("inf")) <= 3
        ]
        if valid_flights:
            return min(
                flight.get("gia_ve_eco", float("inf")) for flight in valid_flights
            )
        return 0
    except:
        return 0
...
def direct_flight_price(flights):
    ...
    try:
        direct_flights = [
            ...
            for f in flights
            if f
            and f.get("san_bay_di")
            and f.get("san_bay_den")
